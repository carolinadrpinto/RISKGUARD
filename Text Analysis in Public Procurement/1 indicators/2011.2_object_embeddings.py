...
contratos = pl.read_csv("../../data/impic_data/contratos.csv", separator=";", schema_overrides=contracts_schema(),
    columns=[
...
contratos_prepro = contratos.join(contratos_cleaned,
                                  how="anti", 
...
for embedding_model, function in zip(embedding_models, embedding_functions):
    ...
    for i, col in enumerate(cols):
        ...
        df_dict = {f"{col}dim{i}": embeddings[:, i].tolist() for i in range(embeddings.shape[1])}
        embeddings_df = pl.DataFrame(df_dict)
